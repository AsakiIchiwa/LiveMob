...
public class RegisterActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        findViewById(R.id.registerBack).setOnClickListener(v -> finish());
        ...
        if (TextUtils.isEmpty(BuildConfig.GOOGLE_WEB_CLIENT_ID)) {
            ...
            btnGoogle.setOnClickListener(v -> googleClient.signOut()
                    .addOnCompleteListener(t -> googleLauncher.launch(googleClient.getSignInIntent())));
        }
    ...
    private void doRegister() {
        ...
        if (email.isEmpty() || password.length() < 6) {
            toast("Email required, password 6+ chars");
            return;
        }
        String displayName = name.isEmpty() ? null : name;
        runBackend(() -> AuthManager.registerSync(this, email, password, displayName));
    }
        ...
        Executors.newSingleThreadExecutor().execute(() -> {
            try {
                ...
                if (resp.isSuccessful() && resp.body() != null) {
                    ...
                    runOnUiThread(() -> toast("Sign-up failed (" + resp.code() + ")"));
                }
            } catch (Exception e) {
